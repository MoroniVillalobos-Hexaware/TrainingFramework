...
Feature:   E Commerce Testing
  ...
  And enter your personal information "adress" and "contact_info"
  ...
  Scenario:Verify Invalid Email address error
    Given open "automation_practice_url"
    When click on sign in link
    And Enter your "email_address" in the email box and click enter
    Then Validate that an error message is displaying saying "invalid Email Address"
  ...
  Scenario:Verify that Wish List Only Works After Login
    ...
    Then Verify That error Message Is Displayed "You Must be Looged In to Manage You wish List"
  ...
  Scenario:Verify that total price is reflecting correctly if user changes quantity in shopping cart
    ...
    And Mouse hover on the second product displayed
    Then  More button will be displayed , click on more button
    Then Make sure quantity is set to 1
    And Select Size M
    And Select Color
    Then Click Add to Car button
